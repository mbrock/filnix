...
let
  ...
  merge = f: g: (old: let step1 = f old; step2 = g (old // step1); in step1 // step2);
  ...
  use = f: overAttrs (
    if builtins.isAttrs f && !builtins.isFunction f
    then (_: f)
    else f
  );
  ...
  src = v: urlf: h:
    use (old:
      let pname = old.pname or (builtins.parseDrvName old.name).name;
      in {
        src     = base.fetchurl { url = urlf v; hash = h; };
        version = v;
        pname   = pname;
        name    = "${pname}-${v}";
      });
  ...
  buildPort = steps:
    let pkg = builtins.head steps;
        rawSteps = builtins.tail steps;
        normalizeStep = step:
          if builtins.isAttrs step && !builtins.isFunction step
          then arg step
          else step;
        normalizedSteps = builtins.map normalizeStep rawSteps;
        acc = foldl' (fa: step: step fa) Init normalizedSteps;
    in fix pkg { deps = acc.deps; attrs = acc.attrs; };

  # Common helpers
  skipTests = use (_: { doCheck = false; doInstallCheck = false; });
  skipCheck = use (_: { doCheck = false; });
  parallelize = use (_: { enableParallelBuilding = true; });
  tool = x: use (old: { nativeBuildInputs = (old.nativeBuildInputs or []) ++ [x]; });
  link = x: use (old: { buildInputs = (old.buildInputs or []) ++ [x]; });
  patch = p: use (old: { patches = (old.patches or []) ++ [p]; });
  skipPatch = p: use (old: { 
    patches = builtins.filter (patch: !(lib.hasInfix p (toString patch))) (old.patches or []); 
  });
  removeCFlag = flag: use (old: {
    env = (old.env or {}) // {
  ...
  github = orgRepo: path: v:
    let parts = lib.splitString "/" orgRepo;
        org = builtins.elemAt parts 0;
        repo = builtins.elemAt parts 1;
    in "https://github.com/${org}/${repo}/releases/download/${path v}";
...
in { 
  inherit buildPort;
  inherit arg use src;
  inherit skipTests skipCheck parallelize tool link patch skipPatch removeCFlag addCFlag configure wip;
