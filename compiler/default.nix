...
{ base, lib, sources, runtime }:

let
  inherit (runtime) libyolo-glibc libyolo libpizlo filc-stdfil-headers libmojo;
  ...
  stage1 = import ./filc1.nix {
    inherit base lib filc0 libyolo-glibc libyolo filc-stdfil-headers;
  };
  inherit (stage1) filc1 filc1-runtime;

  # Stage 2: Compiler with Fil-C runtime (libpizlo)
  stage2 = import ./filc2.nix {
    inherit base lib filc0 libyolo libyolo-glibc libpizlo filc-stdfil-headers;
  };
  ...
  libcxx-build = import ./libcxx.nix {
    inherit base lib sources filc3 libmojo;
  };
  inherit (libcxx-build) filc-libcxx;

  # Rebuild stage3 with libcxx support
  stage3-with-cxx = import ./filc3.nix {
    inherit base filc2 libmojo filc-libcxx;
  };
  inherit (stage3-with-cxx) filc3xx;

in {
  inherit filc0 filc1 filc1-runtime filc2 filc3 filc3xx filc-libcxx;
}
