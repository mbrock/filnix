{
  ...
  outputs = { self, nixpkgs, ... }: let
    ...
    fix = let
      ...
      compiler-stage2 = import ./compiler/filc2.nix {
        inherit base lib;
        inherit (compiler-stage1) filc0;
        inherit (runtime-stage2) libyolo libyolo-glibc libpizlo filc-stdfil-headers;
      };
    ...
    toolchain = import ./toolchain {
      inherit base lib runtime compiler;
    };

    # Port set for ported packages
    portset = import ./portset.nix {
      inherit base;
      inherit (toolchain) filenv withFilC fix filcc;
    ...
    shells = import ./shells {
      inherit base toolchain compiler runtime portset wasm3-cve-payloads ghostty-terminfo dank-bashrc;
    };
    ...
    filc-world-docker = import ./filc-world-docker.nix {
      inherit base portset ghostty-terminfo dank-bashrc;
  ...
  in {
    ...
    packages.${system} = {
      # Compiler stages
      inherit (compiler) filc0 filc1 filc2 filc-libcxx;

      # Runtime libraries
      inherit (runtime) libyolo-glibc libyolo libpizlo libmojo;
