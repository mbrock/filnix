...
tests :-
    ...
    must(Expected==Actual), must(length(Actual,29)),
    ...
    fails(sp_effects:instruction_effects(instruction(movq,[reg(rax),mem(const(0),rdi,none,const(1))]),_),register_destination_required(mem(const(0),rdi,none,const(1)))),
    immediate_limits,
    address_effects,
    trace_tests,
...
check_case(Op,Form,Args,Kind) :-
    ...
    ; must(Control=next), Writes=[write(register(rax,W),integer,Policy)],
    ...
    ; must(Mem=[]),must(Traps=[])),
    (memberchk(Kind,[load,move,return]) -> Flags=flags(reads([]),defined([]),cleared([]),undefined([]),preserved([cf,pf,af,zf,sf,of]))
