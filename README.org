...
5. *filc-glibc* - Build the memory-safe user glibc (heavily modified for safety)
...
#+BEGIN_EXAMPLE
Application Code
├─ filc-glibc (user glibc)    ← Applications link here
├─ libpizlo (Fil-C runtime)   ← Memory safety + FUGC
├─ yolo-glibc (yolo glibc)    ← Runtime uses this
└─ Linux kernel
#+END_EXAMPLE

Both libc implementations are required for a working system. Applications link against filc-glibc, which itself depends on libpizlo for memory safety features, which in turn uses yolo-glibc for low-level system calls.
...
- =filc-glibc= - The memory-safe glibc implementation
