...
[
  ...
  (for "ffi" [
    ...
    (replace "ext/ffi_c/extconf.rb" "libffi_ok &&= have_library(\"ffi\""
  ...
  (for "openssl" [
    ...
    (replace "ext/openssl/ossl.c" "str = rb_protect(ossl_str_new_i, len, &state);"
  ...
  (for "racc" [
    native
    (replace "ext/racc/cparse/cparse.c" "SHIFT(v, act, ERROR_TOKEN, val)"
  ...
  (for "eventmachine" [
    ...
    (replace "ext/rubymain.cpp" "arg = (NIL_P(arg)) ? -1 : NUM2INT (arg);"
      "int limit = (NIL_P(arg)) ? -1 : NUM2INT(arg);"
    )
    (replace "ext/rubymain.cpp" "return INT2NUM (evma_set_rlimit_nofile (arg));"
  ...
  (for "nio4r" [
    ...
    (astGrepAny "#define ECB_MEMORY_FENCE         __asm__ __volatile__ (\"mfence\"   : : : \"memory\")" "#define ECB_MEMORY_FENCE         __atomic_thread_fence(__ATOMIC_SEQ_CST)")
  ...
  (for "curb" [
    ...
    (replace "ext/curb_multi.c" "return method == Qtrue ? 1 : 0;"
