...
This document synthesizes the porting experience of 71 substantial C/C++ projects to Fil-C, Filip Pizlo's memory-safe C implementation. The analysis draws from detailed patch reviews covering everything from small utilities (dash, vim) to massive systems (emacs, python, systemd). The goal: create a reference guide for understanding the core challenges and patterns when porting C/C++ software to a capability-based memory safety model.
