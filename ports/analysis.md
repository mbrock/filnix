...
This document was generated by Claude AI agents analyzing concrete porting changes extracted from Filip Pizlo's [fil-c repository](https://github.com/pizlonator/fil-c), which contains 100+ ported programs in the `projects/` directory. These ports are part of building [Pizlix](https://fil-c.org/pizlix)—a Linux From Scratch 12.2-based distribution with an almost completely memory-safe userland.

**Methodology:** Patches extracted from git history → independent Claude agents analyzed groups of patches → another Claude agent synthesized all reports into this overview.
...
**Key implication:** Any code that treats pointers as integers, manipulates their bits, or makes assumptions about memory layout will break under Fil-C.

## Analyzed Projects

Projects range from small utilities (dash, vim) to massive systems (emacs, python, systemd), spanning:
...
**Patch scope:** ~21,000 lines of changes (build artifacts filtered out during extraction).

### Patch Overview

| Project         | Version      | Lines | Summary                                                                                                                                                                                                                                |
| --------------- | ------------ | ----- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| openssl         | 3.3.1        | 2640  | Removes `-Wl` prefix from symbol versioning flags; adds extensive assembly function wrappers with bounds checking.                                                                                                                     |
| linux           | 6.10.5       | 2292  | VMware GPU driver refactoring: removes dumb buffer surface features with extensive code cleanup.                                                                                                                                       |
| glib            | 2.80.4       | 2252  | Changes `GType` from integer typedef to opaque pointer; adds `uintptr_t` casts throughout.                                                                                                                                             |
| libpng          | 1.6.43       | 1741  | Adds APNG (Animated PNG) support: defines `PNG_APNG_SUPPORTED`, implements frame control chunks (`acTL`, `fcTL`, `fdAT`), animation metadata, and progressive/write APIs.                                                              |
| Python          | 3.12.5       | 1551  | Major refactor: GC linked lists use real pointers, atomic operations use `void*_Atomic`, interpreter frames use GC allocation, `zptrtable` for code object caches, disables SIMD/atomic intrinsics.                                    |
| Linux-PAM       | 1.7.1        | 926   | Replaces hardcoded paths with build-time macros; adds Debian-specific features (both 1.6.1 and 1.7.1 patches).                                                                                                                         |
| perl            | 5.40.0       | 892   | Extensive `zptrtable` integration for pointer encoding across XS modules, internals, and threading; replaces all `PTR2IV`/`INT2PTR` macros with capability-safe encoding.                                                              |
| libuev          | 2.4.1        | 823   | Deletes `.codedocs` CI config.                                                                                                                                                                                                         |
| zlib            | 1.3          | 605   | Disables ARM CRC32 intrinsics; replaces configure-time detection with hardcoded values.                                                                                                                                                |
...
| git             | 2.46.0       | 524   | Converts `intptr_t` casts to/from `void*` in option parsing structures throughout codebase.                                                                                                                                            |
| sqlite          | 3.46.0       | 473   | Adds `zptrtable` for TCL test pointer encoding; disables atomic intrinsics; creates `Makefile.filc` build system; extensive test harness pointer safety fixes.                                                                         |
| cairo           | 1.18.0       | 463   | Replaces GLib's `gsize`-based once initialization with `gpointer` variants for type system compatibility.                                                                                                                              |
| systemd         | 256.4        | 455   | Removes `-Wl,` prefix from version-script linker flags; disables ELF section-based error map registration; inlines all bus error maps into `bus_standard_errors` array; adds signal safety check via `zis_unsafe_signal_for_handlers`. |
| libffi          | 3.4.6        | 440   | Major port: custom closure/trampoline implementation using `zclosure`, custom calling convention (`FFI_FILC`), stack-based argument marshaling, symbol versioning changes.                                                             |
| kbd             | 2.6.4        | 407   | Removes integer casts from `ioctl` calls; fixes keymap definitions (mostly keymap data file corrections).                                                                                                                              |
...
| gettext         | 0.22.5       | 299   | Fixes pointer arithmetic and obstacks; skips two failing tests; symbol lists retain C names for the toolchain-aware libtool probes.                                                                                                                                      |
| binutils        | 2.43.1       | 236   | Converts `intptr_t` variables to `void*` pointers throughout; adds template instantiations; disables `sbrk`; adjusts linker version script flags.                                                                                      |
| weston          | 12.0.5       | 231   | Replaces ELF section-based test registration with constructor-based dynamic allocation in three separate test frameworks.                                                                                                              |
| keyutils        | 1.6.3        | 203   | Replaces raw syscalls with Fil-C syscall wrappers (`zsys_*`); splits install target; removes unused function.                                                                                                                          |
| libuv           | 1.48.0       | 197   | Disables `io_uring`; adds signal safety checks; uses `zexact_ptrtable` for signal handler communication; fixes process title calculation with `zlength`; disables several flaky tests.                                                 |
| xz              | 5.6.2        | 174   | Adds `.filc_symver` symbol versioning support; `zmkptr` for pointer alignment; alignment fix for I/O buffer; disables optimized decoder; fixes linker flag syntax.                                                                     |
| expat           | 2.7.1        | 148   | Adds generated `autoheader` config template file.                                                                                                                                                                                      |
| toybox          | 8.12         | 143   | Build configuration files (`good-config`, `kconfig` binaries) only—no Fil-C compatibility changes.                                                                                                                                     |
| texinfo         | 7.1          | 143   | Fixes pointer alignment macro in `obstack.h`; replaces `intptr_t` with `void*` for proper Fil-C pointer tracking.                                                                                                                      |
| wayland         | 1.24.0       | 134   | Replaces ELF section-based test registration with constructor-based dynamic linked list.                                                                                                                                               |
| libdrm          | 2.4.122      | 127   | Uses `zexact_ptrtable` to encode/decode pointers in DRM `ioctl` structures (most conversions one-way since kernel doesn't return pointers).                                                                                            |
| libinput        | 1.29.1       | 122   | Replaces linker section-based test device and collection registration with constructor-based linked lists; changes `-Wl,--version-script` to `--version-script`.                                                                       |
| sed             | 4.9          | 118   | Disables heap memory mapping checks and malloc size limit tests in gnulib test suite using `__FILC__` guards; adds capability-aware `__BPTR_ALIGN` macro to `obstack.h`.                                                               |
| quickjs         | 2024-02-14   | 107   | Disables direct dispatch; fixes pointer tagging in `JSProperty` autoinit `realm_and_id` field; forces `rqsort` compatibility; adds `stdfil.h` include.                                                                                 |
| e2fsprogs       | 1.47.1       | 106   | Replaces all `sbrk(0)` calls with `NULL` for memory tracking in resource tracking code.                                                                                                                                                |
| zstd            | 1.5.6        | 102   | Uses standard `cpuid.h` for CPU feature detection; disables inline assembly loop alignment optimizations.                                                                                                                              |
| libarchive      | 3.7.4        | 98    | Adds Fil-C tagged pointer operations (`zretagptr`, `zorptr`, `zandptr`, `zxorptr`) to red-black tree implementation; changes pointer-storing deque to use `void*` instead of `size_t`.                                                 |
| libevdev        | 1.11.0       | 89    | Replaces linker section-based test registration with constructor-based linked list; changes `-Wl,--version-script` to `--version-script`.                                                                                              |
| icu             | 76.1         | 77    | Adds early return for timezone; `zmkptr` for alignment; compiler fence for aliasing; disables assembly object code generation.                                                                                                         |
| diffutils       | 3.10         | 77    | Adds Clang diagnostic pragmas to suppress warnings; disables stack overflow recovery.                                                                                                                                                  |
| libevent        | 2.1.12       | 73    | Disables several flaky SSL/HTTPS tests; casts legacy test function pointer to correct signature.                                                                                                                                       |
...
| pcre2           | 10.44        | 48    | Disables locale test in `RunGrepTest`.                                                                                                                                                                                                 |
| libxkbcommon    | 1.11.0       | 47    | Changes `-Wl,--version-script` to `--version-script` in all `meson.build` symbol versioning flags.                                                                                                                                     |
| zsh             | 5.8.0.1-dev  | 46    | Disables zsh's custom heap allocator (`zhalloc`/`hrealloc`) by forcing `malloc`/`free`; adds missing `HashNode` flags field.                                                                                                           |
| krb5            | 1.21.3       | 38    | Removes `-Wl` prefix from version script flag; adds Fil-C fences to libev event loop.                                                                                                                                                  |
| grep            | 3.11         | 38    | Adds `zmkptr` macro for pointer alignment; disables stack overflow recovery in obstack and `sigsegv`.                                                                                                                                  |
| bzip3           | 1.5.1        | 38    | Build system changes: disables pkgconfig, adds version file; includes binary `shakespeare.txt.bz3` file.                                                                                                                               |
| simdutf         | 5.5.0        | 36    | Adds Fil-C support for CPUID detection and `xgetbv` instruction via `zxgetbv()` runtime helper; forces GCC CPUID path.                                                                                                                 |
| libxcrypt       | 4.4.36       | 35    | Disables SSE/SSE2 in yescrypt; changes `.symver` to `.filc_symver` for symbol versioning.                                                                                                                                              |
| Linux-PAM       | 1.6.1        | 34    | Replaces hardcoded paths with build-time macros; adds Debian-specific features (both 1.6.1 and 1.7.1 patches).                                                                                                                         |
| elfutils        | 0.191        | 26    | Adds Clang diagnostic pragmas to suppress unused parameter and unused const variable warnings.                                                                                                                                         |
| tar             | 1.35         | 25    | Adds capability-aware `__BPTR_ALIGN` macro to `gnu/obstack.h` using `zmkptr` for pointer alignment operations (identical to sed patch).                                                                                                |
| lfs-bootscripts | 20240825     | 25    | Creates `/run/user` directory at boot; enables unlimited core dumps for debugging.                                                                                                                                                     |
| jpeg            | 6b           | 25    | Changes alignment type from `void*` to `double`; removes test binary images.                                                                                                                                                           |
| bison           | 3.8.2        | 25    | Modifies pointer alignment macro in `obstack.h` to use `zmkptr` for proper capability preservation.                                                                                                                                    |
...
| lua             | 5.4.7        | 21    | Disables readline library; adds debug flags to makefile.                                                                                                                                                                               |
| procps-ng       | 4.0.4        | 19    | Fixes `va_arg` undefined behavior in `pids_oldproc_open` by conditionally reading pointer argument only when flags indicate it's present.                                                                                              |
| attr            | 2.5.2        | 18    | Changes symbol versioning directives from `.symver` to `.filc_symver` for Fil-C linker compatibility.                                                                                                                                  |
| tmux            | 3.5a         | 16    | No Fil-C compatibility changes.                                                                                                                                                                                                        |
| cmake           | 3.30.2       | 16    | Changes default library directory from `lib64` to `lib` for 64-bit systems.                                                                                                                                                            |
...
| XML-Parser      | 2.47         | 13    | Uses `zptrtable_decode` to properly recover Perl XS pointer from integer representation.                                                                                                                                               |
| vim             | 9.1.0660     | 13    | Fixes `sigaction` call to pass `NULL` when func is `SIG_ERR` to prevent dereferencing invalid pointer.                                                                                                                                 |
| seatd           | 0.9.1        | 13    | Changes linker version script flag from `-Wl,--version-script` to bare `--version-script` in `meson.build`.                                                                                                                            |
...
| dash            | 0.5.12       | 13    | Replaces `vfork` with `fork`.                                                                                                                                                                                                          |
| bash            | 5.2.32       | 13    | Changes flexible array member from `char` to `void*` in `SAVED_VAR` struct for proper pointer table handling.                                                                                                                          |
...
These patterns are **sufficient** to port substantial codebases — from text editors (vim, emacs) to language runtimes (python, perl) to system services (systemd, openssh).
